...
import { carPageStyles } from "../../../public/assets/dummyStyles";
...
const CarPage = () => {
  ...
  // base URL for local app integration
  ...
  const fetchCars = async () => {
    ...
    try {
      ...
      const json = res.data;
      const loadedCars = Array.isArray(json?.data)
        ? json.data
        : Array.isArray(json)
          ? json
          : Array.isArray(json?.cars)
            ? json.cars
            : [];

      setCars(loadedCars.length ? loadedCars : carsData);
    } catch (err) {
      const isCanceled =
      ...
      setCars(carsData);
      setError(
        "Showing demo vehicles because the live service is unavailable."
      );
    } finally {
      setLoading(false);
    }
