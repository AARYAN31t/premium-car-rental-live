...
import { carDetailStyles } from "../../../public/assets/dummyStyles";
...
const CarDetailPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userId = storedUser?.id ?? "demo-user";
      const token = localStorage.getItem("token");
      const payload = {
        userId,
        customer: formData.name || storedUser?.name || "Demo User",
        email: formData.email || storedUser?.email || "demo@carzone.com",
      ...
      try {
        ...
        console.warn("Remote checkout unavailable, saving booking locally.", remoteErr);
      }
      ...
      existingBookings.unshift(bookingEntry);
      localStorage.setItem("demoBookings", JSON.stringify(existingBookings));

      toast.success(
        "Booking saved locally. You can view it in My Bookings.",
